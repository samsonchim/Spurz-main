...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const coversDir = path.join(process.cwd(), 'public', 'outlets', 'covers')
    ...
    const coverPath = `/outlets/covers/${filename}`

    const { data, error } = await supabaseAdmin
      .from('outlets')
      .update({ cover_photo_path: coverPath, updated_at: new Date().toISOString() })
      .eq('id', outletId)
      .select()
      .single()
    ...
    if (error) {
      ...
      return res.status(500).json({ ok: false, error: 'Failed to update outlet cover' })
    }

    return res.status(200).json({ ok: true, cover_photo_path: coverPath, outlet: data })
  } catch (e: any) {
